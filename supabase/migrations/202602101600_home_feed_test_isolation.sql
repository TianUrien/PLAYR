-- ============================================================================
-- Migration: Home Feed Test Account Isolation
-- Date: 2026-02-10
-- Description: Filters out user posts from test accounts in get_home_feed().
--   System feed items (home_feed_items) are already protected at trigger level.
--   User posts were missing the is_test_account filter, allowing test account
--   posts to leak into real users' feeds.
-- ============================================================================
...
BEGIN
  ...
  IF p_item_type = 'user_post' THEN
    ...
    FROM (
      ...
      WHERE up2.deleted_at IS NULL
        AND (p2.is_test_account IS NULL OR p2.is_test_account = false)
  ...
  -- Count total from both sources (excluding test user posts)
  SELECT (
  ...
  FROM (
    ...
    FROM (
      ...
      -- User posts (exclude test account authors)
      SELECT
        up.created_at,
        jsonb_build_object(
          'feed_item_id', up.id,
...
COMMENT ON FUNCTION public.get_home_feed IS 'Fetches paginated home feed with system posts + user posts unified (test accounts excluded)';
