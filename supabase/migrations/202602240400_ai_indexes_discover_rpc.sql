-- ============================================================================
-- Migration: AI search indexes + discover_profiles RPC
-- ============================================================================
-- Creates composite partial indexes optimized for AI query patterns and the
-- discover_profiles RPC that the NL search edge function calls.
-- ============================================================================

-- ============================================================================
-- 1. COMPOSITE INDEXES FOR AI FILTER QUERIES
-- ============================================================================
-- All partial indexes exclude incomplete/test/blocked profiles since the AI
-- search RPC always applies these base conditions.

-- Role + Position + Gender (most common AI filter combination)
CREATE INDEX IF NOT EXISTS idx_profiles_ai_role_pos_gender
  ON profiles (role, position, gender)
  WHERE onboarding_completed = true AND is_test_account = false AND is_blocked = false;

-- Age range queries (U21, 18-24, etc.) — needs date_of_birth
CREATE INDEX IF NOT EXISTS idx_profiles_ai_dob
...
-- Reference-based ranking ("2+ references", "most referenced")
CREATE INDEX IF NOT EXISTS idx_profiles_ai_ref_count
  ON profiles (accepted_reference_count DESC)
  WHERE onboarding_completed = true AND is_test_account = false AND is_blocked = false;

-- Availability filter ("open to play", "available")
CREATE INDEX IF NOT EXISTS idx_profiles_ai_availability
  ON profiles (role, open_to_play, open_to_coach)
  WHERE onboarding_completed = true AND is_test_account = false AND is_blocked = false;

-- Recently active sorting
CREATE INDEX IF NOT EXISTS idx_profiles_ai_last_active
  ON profiles (last_active_at DESC NULLS LAST)
  WHERE onboarding_completed = true AND is_test_account = false AND is_blocked = false;

-- Base city for ILIKE searches
CREATE INDEX IF NOT EXISTS idx_profiles_ai_base_city
  ON profiles (base_city)
  WHERE base_city IS NOT NULL
    AND onboarding_completed = true
    AND is_test_account = false
    AND is_blocked = false;

-- ============================================================================
-- 2. DISCOVER_PROFILES RPC
-- ============================================================================
-- Purpose: Single RPC for the AI search edge function. Accepts all structured
-- filter parameters and returns enriched profiles with aggregate counts,
-- nationality names, and flag emojis.
--
...
BEGIN
  ...
  WHERE p.onboarding_completed = true
    ...
    -- Nationality filter (specific country IDs)
    ...
    -- Base location text (ILIKE on base_city or base_location)
  ...
  FROM (
    ...
    ORDER BY
      CASE p_sort_by
        WHEN 'newest' THEN NULL -- handled by created_at below
        WHEN 'most_referenced' THEN NULL -- handled by ref count below
        WHEN 'recently_active' THEN NULL -- handled by last_active below
        ELSE NULL -- relevance: FTS rank or newest
      END,
...
-- but also available for direct RPC calls from authenticated clients)
...
COMMENT ON FUNCTION public.discover_profiles IS
  'AI search RPC: accepts structured filter parameters, returns enriched profiles with '
  'aggregate counts, nationality names, and flag emojis. All parameters optional — '
  'NULL means no filter. Called by the nl-search edge function.';
