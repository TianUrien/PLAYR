-- ============================================================================
-- FIX INVESTOR DASHBOARD TABLE REFERENCES
-- ============================================================================
-- Updates the investor dashboard RPC functions to use the new table names
-- after the terminology alignment migration:
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    -- Engagement signals (FIXED: use new table names)
    'dau_7d_avg', (
...
BEGIN
  ...
  -- Return metrics (FIXED: use new table names)
  SELECT json_build_object(
    -- User totals (excluding test accounts)
    'total_users', (SELECT COUNT(*) FROM profiles WHERE NOT is_test_account),
    'total_players', (SELECT COUNT(*) FROM profiles WHERE role = 'player' AND NOT is_test_account),
