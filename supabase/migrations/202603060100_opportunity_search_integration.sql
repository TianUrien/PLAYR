-- ============================================================================
-- Migration: Opportunity Search Integration
-- Date: 2026-03-06
-- Description: Adds opportunities to the global search system.
--   1. Adds search_vector column + GIN index + auto-update trigger to opportunities
--   2. Updates search_content() RPC to include an opportunities search block
-- ============================================================================

SET search_path = public;

-- ============================================================================
-- 1. Add search_vector to opportunities table
-- ============================================================================

ALTER TABLE public.opportunities ADD COLUMN IF NOT EXISTS search_vector tsvector;
...
-- Auto-update trigger
CREATE OR REPLACE FUNCTION public.update_opportunities_search_vector()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
DROP TRIGGER IF EXISTS trigger_update_opportunities_search ON public.opportunities;
CREATE TRIGGER trigger_update_opportunities_search
  BEFORE INSERT OR UPDATE OF title, description, organization_name, location_city, location_country, position
  ON public.opportunities
  FOR EACH ROW
  EXECUTE FUNCTION public.update_opportunities_search_vector();

-- GIN index for fast full-text search
CREATE INDEX IF NOT EXISTS idx_opportunities_search ON public.opportunities USING GIN (search_vector);

-- ============================================================================
-- 2. Update search_content() RPC — add opportunities search block
-- ============================================================================
...
BEGIN
  ...
  IF p_type IS NULL OR p_type = 'opportunities' THEN
    ...
    WHERE o.status = 'open'
      AND o.search_vector @@ v_tsquery
      AND (v_is_test OR cp.is_test_account IS NULL OR cp.is_test_account = false);

    SELECT COALESCE(jsonb_agg(row_data ORDER BY rank DESC), '[]'::jsonb)
    INTO v_opportunity_results
    FROM (
      SELECT
        jsonb_build_object(
          ...
          'club_avatar_url', cp.avatar_url,
          'published_at', o.published_at
        ) AS row_data,
        ts_rank(o.search_vector, v_tsquery) AS rank
      FROM opportunities o
      JOIN profiles cp ON cp.id = o.club_id
      WHERE o.status = 'open'
        AND o.search_vector @@ v_tsquery
  ...
  ELSIF p_type = 'opportunities' THEN
    v_results := COALESCE(v_opportunity_results, '[]'::jsonb);
  ELSE
    v_results := COALESCE(v_post_results, '[]'::jsonb)
...
COMMENT ON FUNCTION public.search_content IS 'Full-text search across posts, people, clubs, brands, and opportunities. Returns combined results with type counts. Respects test account visibility.';
