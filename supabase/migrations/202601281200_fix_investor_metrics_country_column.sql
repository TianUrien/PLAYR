-- ============================================================================
-- FIX INVESTOR METRICS COUNTRY COLUMN REFERENCE
-- ============================================================================
-- The profiles table uses 'nationality' not 'country'. Fix the investor
-- metrics functions to use the correct column name.
-- ============================================================================
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    -- Top countries (FIXED: use nationality column)
    'top_countries', (
    ...
    'total_applications_30d', (SELECT COUNT(*) FROM opportunity_applications WHERE created_at > v_now - interval '30 days'),
...
BEGIN
  ...
  -- Return metrics (FIXED: use nationality column)
  SELECT json_build_object(
    -- User totals (excluding test accounts)
    'total_users', (SELECT COUNT(*) FROM profiles WHERE NOT is_test_account),
    'total_players', (SELECT COUNT(*) FROM profiles WHERE role = 'player' AND NOT is_test_account),
