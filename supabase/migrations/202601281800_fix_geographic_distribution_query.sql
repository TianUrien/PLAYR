-- ============================================================================
-- FIX GEOGRAPHIC DISTRIBUTION - USE NORMALIZED COUNTRY DATA
-- ============================================================================
-- The investor metrics functions were using p.nationality (free-text) instead
-- of joining to the countries table via nationality_country_id.
-- This caused duplicates like "Argentina", "argentina", "Argentino", blanks.
-- ============================================================================
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    -- Top countries (FIXED: use normalized country data via nationality_country_id)
    'top_countries', (
      SELECT COALESCE(json_agg(row_to_json(geo)), '[]'::json)
      FROM (
        SELECT
...
COMMENT ON FUNCTION public.admin_get_investor_metrics IS 'Returns investor dashboard metrics (admin only) - uses normalized country data';
...
BEGIN
  ...
  -- Return metrics (FIXED: use normalized country data via nationality_country_id)
  SELECT json_build_object(
    -- User totals (excluding test accounts)
    'total_users', (SELECT COUNT(*) FROM profiles WHERE NOT is_test_account),
    'total_players', (SELECT COUNT(*) FROM profiles WHERE role = 'player' AND NOT is_test_account),
...
COMMENT ON FUNCTION public.public_get_investor_metrics IS 'Returns investor dashboard metrics via shareable token - uses normalized country data';
