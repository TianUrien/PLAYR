-- Fix: get_club_members must match ALL world_clubs claimed by a profile,
-- not just one. A club can claim multiple world_club entries (e.g. men's + women's).
-- The original LIMIT 1 caused members linked to any non-first club to be invisible.
