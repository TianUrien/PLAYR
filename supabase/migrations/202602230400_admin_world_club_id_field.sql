-- ============================================================================
-- Add current_world_club_id to admin_update_profile allowed fields
-- Allows admins to link/unlink players and coaches to World clubs
-- via the Admin Portal → Edit User Profile modal.
-- ============================================================================
...
BEGIN
  ...
  -- current_club and current_world_club_id use CASE WHEN (? operator)
  -- so they can be explicitly set to NULL (clearing a club link).
  -- Other fields use COALESCE (existing pattern, backward-compatible).
  UPDATE profiles
  SET
...
COMMENT ON FUNCTION public.admin_update_profile IS 'Updates profile fields with audit logging (includes current_world_club_id)';
