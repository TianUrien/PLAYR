-- =============================================================================
-- World Club Avatars: add avatar_url column + storage bucket
--
-- Allows admins to upload club logos directly. When a club is claimed,
-- the world club's own avatar takes priority over the profile avatar.
-- =============================================================================

-- 1. Add avatar_url column to world_clubs
ALTER TABLE world_clubs ADD COLUMN IF NOT EXISTS avatar_url TEXT;

-- 2. Create storage bucket for world club logos
INSERT INTO storage.buckets (id, name, public)
VALUES ('world-club-logos', 'world-club-logos', true)
ON CONFLICT (id) DO NOTHING;

-- 3. Storage RLS policies
CREATE POLICY "world_club_logos_public_read"
ON storage.objects FOR SELECT
USING (bucket_id = 'world-club-logos');

CREATE POLICY "world_club_logos_auth_insert"
ON storage.objects FOR INSERT
WITH CHECK (bucket_id = 'world-club-logos' AND auth.role() = 'authenticated');

CREATE POLICY "world_club_logos_auth_update"
ON storage.objects FOR UPDATE
USING (bucket_id = 'world-club-logos' AND auth.role() = 'authenticated');

CREATE POLICY "world_club_logos_auth_delete"
ON storage.objects FOR DELETE
USING (bucket_id = 'world-club-logos' AND auth.role() = 'authenticated');

-- 4. Update search_content RPC: prefer world club avatar over profile avatar
    ...
    FROM (
      SELECT
        jsonb_build_object(
          ...
          'avatar_url', COALESCE(wc.avatar_url, p.avatar_url),
...
COMMENT ON FUNCTION public.search_content IS 'Full-text search across posts (tsvector prefix), people (tsvector prefix + ILIKE fallback), and clubs (ILIKE). Returns combined results with type counts. Respects test account visibility. Club avatars use COALESCE(world_club avatar, profile avatar).';

-- 5. Update search_clubs_for_transfer RPC: prefer world club avatar
...
COMMENT ON FUNCTION public.search_clubs_for_transfer IS 'Searches world_clubs by name for transfer announcements. Returns club info with country and avatar. Club avatars use COALESCE(world_club avatar, profile avatar).';
